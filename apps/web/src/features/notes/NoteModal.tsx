...
export function NoteModal({ isOpen, onClose, noteId }: NoteModalProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('orqelis:save-note', handleSaveShortcut);
    window.addEventListener('orqelis:toggle-note-preview', handlePreviewShortcut);
    window.addEventListener('orqelis:close-note-modal', handleCloseShortcut);

    return () => {
      window.removeEventListener('orqelis:save-note', handleSaveShortcut);
