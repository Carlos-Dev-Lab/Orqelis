...
export function NotesView() {
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)] flex flex-col overflow-hidden bg-background">
      {/* Header */}
      <header className="px-4 @2xl:px-6 py-4 @2xl:py-5 border-b border-on-surface/5">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 @2xl:gap-4 min-w-0">
            <div className="w-10 h-10 @2xl:w-12 @2xl:h-12 shrink-0 rounded-2xl bg-primary/10 flex items-center justify-center">
              <FileText className="w-6 h-6 text-primary" />
            </div>
            <div className="min-w-0">
              <h1 className="text-xl @2xl:text-2xl font-bold text-on-surface truncate">{t('notes', language)}</h1>
        ...
        {/* Filters and Search */}
        <div className="mt-4 @2xl:mt-5 flex items-center gap-2 @2xl:gap-3">
            ...
            {activeGroupId && (
              <Tooltip content={activeGroup?.name} color={activeGroup?.color}>
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 bg-primary/10 text-primary rounded-2xl border border-primary/20 text-sm font-medium max-w-[140px] @2xl:max-w-[200px]"
                >
      ...
      {/* Content */}
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 @2xl:p-6">
        {filteredNotes.length > 0 ? (
          <div className={cn(
            "grid gap-4 @2xl:gap-6",
            viewMode === 'grid' 
              ? "grid-cols-1 @xl:grid-cols-2 @4xl:grid-cols-3 @6xl:grid-cols-4 @9xl:grid-cols-5"
              : "grid-cols-1"
          )}>
