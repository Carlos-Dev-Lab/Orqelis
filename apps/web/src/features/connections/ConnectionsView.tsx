...
export function ConnectionsView() {
  ...
  return (
    <div className="h-full flex flex-col bg-surface-container-lowest overflow-hidden">
      <header className="px-4 @2xl:px-6 py-4 border-b border-on-surface/5 bg-surface-container-lowest/50 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-lg @2xl:text-xl font-bold text-on-surface truncate">{t('connectionsAndAccess', language)}</h2>
              <p className="text-xs text-on-surface-variant truncate">{connections.length} {t('connectionsAndAccess', language).toLowerCase()}</p>
            </div>
          </div>
          <button
            onClick={handleOpenNew}
            className="flex shrink-0 items-center gap-2 whitespace-nowrap px-3 @md:px-4 py-2 bg-primary text-on-primary rounded-xl hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20 text-sm font-medium"
            title={t('newConnection', language)}
          >
            <Plus className="w-4 h-4" />
            <span className="hidden @md:inline">{t('newConnection', language)}</span>
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 @2xl:p-6 custom-scrollbar">
        {filteredConnections.length === 0 ? (
          ...
          <div className="grid grid-cols-1 @xl:grid-cols-2 @4xl:grid-cols-3 @6xl:grid-cols-4 @9xl:grid-cols-5 gap-4 items-start">
