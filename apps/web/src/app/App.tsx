...
export default function App() {
  ...
  // Collapse sidebar by default below lg, where the expanded sidebar overlays the content
  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (window.matchMedia('(max-width: 1023px)').matches && !useAppStore.getState().sidebarCollapsed) {
      toggleSidebar();
    }
  ...
  return (
    <>
      ...
      <TopBar />
      <main className={cn(
        "@container/main pt-14 h-screen transition-all duration-300",
        sidebarCollapsed ? "pl-0 md:pl-[72px]" : "pl-0 md:pl-[72px] lg:pl-[280px]"
      )}>
        {renderView()}
      </main>
    </>
