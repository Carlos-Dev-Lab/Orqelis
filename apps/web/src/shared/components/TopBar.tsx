...
export function TopBar() {
  ...
  return (
    <header className={cn(
      "@container fixed top-0 right-0 left-0 h-14 bg-surface/60 backdrop-blur-xl border-b border-on-surface/5 z-40 flex items-center justify-between gap-3 px-4 sm:px-6 transition-all duration-300",
      sidebarCollapsed ? "md:left-[72px]" : "md:left-[72px] lg:left-[280px]"
    )}>
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <button
          ...
          className="md:hidden shrink-0 p-2 -ml-1 rounded-lg hover:bg-on-surface/5 text-on-surface-variant transition-colors"
          aria-label={t('toggleSidebarLabel', language)}
        >
          <Menu className="w-5 h-5" />
        </button>
        <h2 className="min-w-0 text-base sm:text-lg font-semibold text-on-surface truncate">
          {viewTitles[currentView]}
          {activeWorkspace && (
            <span className="ml-2 text-xs font-normal text-on-surface-variant hidden @2xl:inline-flex items-center gap-1.5">
        ...
        {currentView !== 'landing' && (
          <div className="hidden @4xl:flex shrink-0 items-center gap-2 text-xs font-mono text-on-surface-variant/60">
            <span className={cn(
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <button
          ...
          className="hidden @lg:flex items-center gap-1 whitespace-nowrap px-2 py-1 rounded-lg bg-on-surface/5 border border-on-surface/10 text-on-surface-variant text-sm hover:bg-on-surface/10 hover:border-on-surface/20 transition-all"
        >
          <Search className="w-4 h-4" />
          <span className="hidden @2xl:inline">{t('searchNotes', language)}</span>
          <kbd className="hidden @3xl:inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-mono bg-on-surface/10 rounded border border-on-surface/10">
            <span>Ctrl</span>
            <span>+</span>
            <span>K</span>
          </kbd>
        ...
        <button 
          onClick={() => setCommandPaletteOpen(true)}
          className="@lg:hidden p-2 rounded-lg hover:bg-on-surface/5 text-on-surface-variant transition-colors"
        >
        ...
        {isAuthenticated ? (
          <button 
            ...
            className="flex items-center gap-3 p-1.5 @2xl:pr-3 rounded-lg hover:bg-on-surface/5 text-on-surface transition-all group"
            title="Profile"
          >
          ...
          <button 
            ...
            className="flex items-center gap-3 p-1.5 @2xl:pr-3 rounded-lg hover:bg-primary/10 text-on-surface transition-all group"
          >
