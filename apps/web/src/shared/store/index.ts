...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  noteModalOpen: false,
  setNoteModalOpen: (open) => set({ noteModalOpen: open }),
  openNewNote: () => {
...
if (typeof window !== 'undefined') {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (isMod && e.key.toLowerCase() === 's' && state.noteModalOpen) {
      e.preventDefault();
      window.dispatchEvent(new Event('orqelis:save-note'));
      return;
    }

    if (isMod && e.key.toLowerCase() === 'p' && state.noteModalOpen) {
      e.preventDefault();
      window.dispatchEvent(new Event('orqelis:toggle-note-preview'));
      return;
    }
    ...
    if (e.key === 'Escape') {
      if (state.commandPaletteOpen) {
        ...
        state.setWorkspaceManagerOpen(false);
      } else if (state.groupManagerOpen) {
        state.setGroupManagerOpen(false);
      } else if (state.alertDialog.show) {
        state.hideAlert();
      } else if (state.noteModalOpen) {
        window.dispatchEvent(new Event('orqelis:close-note-modal'));
      }
    }
  }, true);
}
